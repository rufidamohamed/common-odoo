...
class SMSComposer(models.TransientModel):
    ...
    def send_sms_action(self):
        ...
        for ids in active_ids:
            ...
            message = self.env['send.sms.template'].render_template(self.body_text, my_model, ids)
            mobile_no = self.env['send.sms.template'].render_template(self.sms_to_lead, my_model, ids)
            self.env['send.sms.template'].send_sms_link(message, mobile_no,ids,my_model,self.gatewayurl_id)
        return True
